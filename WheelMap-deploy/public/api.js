...
app.post('/search', (req, res) => {
  try {
    ...
    db.query(
      `SELECT * FROM charge WHERE CONCAT(facility_name, sido_name, sigungu_code, road_address, longitude, providing_agency_code) LIKE ?`,
      [`%${searchTerm}%`],
      (err, results) => {
        if (err) {
          console.error(err);
...
app.post('/like', (req, res) => {
  ...
  db.query('SELECT id FROM userinfo WHERE id = ?', [user_id], (err, userResults) => {
    ...
    db.query('SELECT id FROM charge WHERE id = ?', [store_id], (err, storeResults) => {
      if (err) {
        console.error(err.message);
...
app.get(`/liked-places`, (req, res) => {
  ...
  db.query('select * from likes l inner join charge i on l.store_id = i.id where user_id = ?', [userId], (err, results) => {
    if (err) {
      console.error(err.message);
